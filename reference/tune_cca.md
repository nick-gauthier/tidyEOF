...
- metrics:
  ...
  "cor_spatial", "cor_temporal" (default: all three). For multivariate
  fields each metric also gets per-variable columns (e.g.
  \`rmse_tmean\`); the plain name is the pooled score (sd-normalized RMS
  for \`rmse\`, mean for correlations).
