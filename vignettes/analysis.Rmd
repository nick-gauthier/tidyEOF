...
```{r}
nmodes <- 4
prism_patterns <- get_patterns(prism_dat, nmodes, prism_mask)
```
...
# find a way not to have to hard code this
prism_patterns$amplitudes <- prism_patterns$amplitudes %>%
  mutate(amplitude = if_else(PC %in% c('1','2'), amplitude * -1, amplitude))
```
...
```{r}
full_join(prism_patterns$amplitudes, prism_dat, by = 'year') %>%
  group_by(x, y, PC) %>%
  summarise(correlation = cor(amplitude, SWE)) %>%
   ...
   ggplot() +
    ...
    scale_fill_scico(palette = 'broc', limits = c(-1, 1)) +
  theme_void()+
...
prism_patterns$eofs %>%
  ...
  left_join(prism_patterns$climatology) %>%
  ggplot() +
      geom_raster(aes(x, y, fill = weight/swe_sd)) +
      geom_sf(data = states_wus, fill = NA, color = 'black') +
      facet_wrap(~EOF, ncol = 2) +
      scale_fill_distiller(name = 'weight/sdev', palette = 'BrBG', direction = 1, limits = c(-1, 1) ) +
      theme_void()+
...
```{r, warning = FALSE}
sst_cor <- map(1:nmodes, ~calc(sst_jfm, fun=function(x) cor(c(x), prism_patterns[[.,2]]$amplitude))) %>%
  brick() %>%
...
```{r}
cera_patterns <- get_patterns(cera_dat, k = 6, mask = FALSE)
```

```{r, echo = FALSE}
plot_amps(cera_patterns) +geom_vline(xintercept = 1977)
```
