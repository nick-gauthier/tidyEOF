...
```{r}
sst_mean <- mean(brick('../data/sst.mon.ltm.1981-2010.nc')[[1:3]])

sst_brick <- brick('../data/sst.mnmean.nc')
...
sst_cor <- brick(
  ...
  calc(sst_jfm, fun=function(x) cor(c(x),prism_patterns[[5,2]]$amplitude)),
  calc(sst_jfm, fun=function(x) cor(c(x),prism_patterns[[6,2]]$amplitude))) %>%
  `names<-`(paste0('PC', 1:6))
```
