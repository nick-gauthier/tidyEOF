...
```{r}
prism_patterns <- get_patterns(prism_dat, k = nmodes, mask = prism_mask)
cera_patterns <- get_patterns(cera_dat, k = nmodes, mask = cera_mask)
```
...
prism_recon %>%
  left_join(prism_dat, by = c('x', 'y', 'year'), suffix = c('_recon', '_obs')) %>%
  mutate(residuals = SWE_recon - SWE_obs) %>%
  group_by(x, y) %>%
  summarise(avg = mean(residuals),
            stdev = sd(residuals)) %>%
    ggplot() +
...
prism_recon %>%
  left_join(prism_dat, by = c('x', 'y', 'year'), suffix = c('_recon', '_obs')) %>%
  group_by(x, y) %>%
  summarise(rmse = sqrt(mean((SWE_obs - SWE_recon) ^ 2, na.rm = TRUE))) %>% 
  ggplot() +
  geom_raster(aes(x, y, fill = rmse)) +
  geom_sf(data = states_wus, fill = NA, color = 'black') +
  theme_void() +
...
prism_recon %>%
  left_join(prism_dat, by = c('x', 'y', 'year'), suffix = c('_recon', '_obs')) %>%
  group_by(x, y) %>%
  summarise(corr = cor(SWE_obs, SWE_recon)) %>% 
  ggplot() +
  geom_raster(aes(x, y, fill = corr)) +
  geom_sf(data = states_wus, fill = NA, color = 'black') +
  theme_void() +
...
test %>%
  ...
  summarise(rmse = sqrt(mean((SWE_recon - SWE_obs)^2, na.rm = TRUE)),
            ...
            srmse = rmse/sd) %>% # aka rsr
   ggplot() +
      geom_raster(aes(x, y, fill = srmse)) + # gigaliters?
      geom_sf(data = states_wus, fill = NA, color = 'black') +
      scale_fill_viridis_c() +
      theme_void()
