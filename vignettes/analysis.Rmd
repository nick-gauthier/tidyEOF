...
```{r}
couple_patterns(cera_patterns, prism_patterns)  %>%
...
prep_kfold <- function(data){
  data %>%
      mutate(PC = paste0(PC, 'response')) %>%
      spread(PC, amplitude) %>%
      crossv_kfold(5) %>%
      mutate(
        train = map(
          train,
          ~ as_tibble(.) %>% gather(PC, amplitude, contains('response')) %>% mutate(PC = str_remove(PC, 'response'))
        ),
        test = map(test, ~ as_tibble(.) %>% dplyr::select(-contains('response')))
      )
}

fit_kfold <- function(kfold, obs_val) {
  kfold %>%
      mutate(test_mod = map(train, fit_model)) %>%
      mutate(test_result = map2(test_mod, test, ~ predict_patterns(.x, .y) %>% 
            reconstruct_field(obs_val, .)
      ))
}
```
...
test_dat <- expand_grid(k_cera = exp_test$k_cera, k_prism = exp_test$k_prism) %>%
  mutate(data = map2(k_cera, k_prism, ~prep_data(exp_test[[.x, 3]], exp_test[[.y, 4]])))  %>%
  mutate(kfold = map(data, prep_kfold)) %>%
  mutate(fit_kfold = map2(kfold, k_prism, ~fit_kfold(.x, exp_test[[.y, "prism_patterns"]])))
```
...
rmse <- test2 %>%
      ...
      group_by(year) %>%
      summarise(total_recon = sum(SWE_recon),
                total_obs = sum(SWE_obs)) %>%
      summarise(sqrt(mean((
        total_obs - total_recon
      ) ^ 2))) %>% pull
```
