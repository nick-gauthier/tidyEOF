...
map2_dfr(mod, predictors, predict_cv) %>%
  arrange(PC, year)
```
...
```{r}
t1  
  map(t1, ~plot_eofs(., scaled = FALSE) )
map(t1, plot_amps)
t1[[2]] %>% plot_amps()

map(t1, ~.$amplitudes)
map_dfr(t1, ~.$amplitudes, .id = 'fold') %>% ggplot(aes(year, abs(amplitude), color = fold)) + geom_point() + facet_wrap(~PC)
# 
```

```{r}
cv_dat <- expand_grid(k_predictor = 1:9, k_response = 1:9) %>%
  mutate(cv = map2(k_predictor, k_response, fit_kfold))
```
...
tmp1 <- scores %>%
  select(k_prism, k_cera, score_ts) %>%
  unnest(score_ts) %>%
...
```{r}
tmp2 <- scores %>%
  select(k_prism, k_cera, score_map) %>%
  unnest(score_map) %>%
  select(k_prism:y, rmse, mdsa, srmse, rmsle) %>%
  group_by(k_prism, k_cera) %>%
