...
cera_patterns <- get_patterns(cera_dat %>% filter(year >= 1982), k = n_modes)
cesm_patterns <- get_patterns(cesm_dat, k = n_modes)
ccsm_patterns <- get_patterns(ccsm_dat, k = n_modes)
common_patterns_cesm <- get_patterns(filter(cera_dat, between(year, 1951, 2000))%>% semi_join(get_climatology(cesm_dat)), 
                                 k = n_modes, 
                                 common = filter(cesm_dat, year < 1951 & year >= 1901))

common_patterns_ccsm <- get_patterns(filter(cera_dat, between(year, 1901, 2005)),
                                 k = n_modes, 
                                 common = filter(ccsm_dat, between(year, 1901, 2005)) %>% mutate(year = year - 105))
```
...
plot_amps(common_patterns_cesm)
plot_amps(common_patterns_ccsm)
```

```{r}
plot_eofs(prism_patterns, 'vik', scaled = FALSE)
plot_eofs(cesm_patterns, scaled = FALSE)
plot_eofs(common_patterns_cesm, 'vik', scaled = FALSE)

plot_eofs(cera_patterns, 'vik', scaled = FALSE)
plot_eofs(common_patterns_ccsm, 'vik', scaled = FALSE)
plot_eofs(ccsm_patterns, 'vik', scaled = FALSE)
```
...
prism_dat %>%
  ...
  mutate(anomaly = (SWE - mean(SWE)) / sd(SWE)) %>%
  ungroup() %>%
  filter(year %in% extreme_years) %>%
  mutate(year = factor(year, levels = extreme_years)) %>%
  ggplot(aes(x, y, fill = anomaly)) +
  geom_raster() +
  facet_wrap(~year) +
  coord_quickmap() +
  scale_fill_distiller(palette = 'BrBG', direction = 1, limits = c(-6, 6)) +
  theme_void()
```
...
clim_baseline <- t2 %>%
  ...
  rename(SWE = swe_mean) %>%
  get_errors  %>% 
   left_join(areas_prism, by = c("x", "y")) %>%
    group_by(year) %>%
...
```{r}
cv_recons <- cv_dat %>%
  mutate(cv = map(cv, ~left_join(., areas_prism, by = c("x", "y")))) %>%
  unnest(cv) %>%
 # mutate(covered = if_else(SWE >= 3, area, 0)) %>%
      mutate(covered = pmin(1, SWE / 3) * area) %>% # linear decline below 3mm
...
```{r}
# why doesn't CESM work here? there's probably a null cell at the edge that is in cera. might have to mask cera too?
...
# need a way for this to couple for patterns with different time horizons
test <- get_patterns(cera_dat, 5, common = filter(ccsm_dat, year >= 1751)) 
# try doing this so you predict on the common eof data too, so you can see whether it messes up the reanalysis period and what the change in mean looks like
test$amplitudes <- filter(test$amplitudes, year >=1982)

recon_common_ccsm <- predict_cca(test,
              get_patterns(filter(prism_dat, year <= 2010), 4), 
            filter(ccsm_dat, year >1700),
            k = 4)

test2 <- get_patterns(cera_dat%>% semi_join(get_climatology(cesm_dat)), 5, common = filter(cesm_dat, year >= 1701 & year <= 1900))
test2$amplitudes <- filter(test2$amplitudes, year >=1982)

recon_common_cesm <- predict_cca(test2,
              get_patterns(filter(prism_dat, year <= 2010), 4), 
            filter(cesm_dat, year > 1700),
            k = 4)

# why is there a mean bias?
recon_project_ccsm %>%group_by(year) %>% summarise(SWE = sum(SWE)) %>%
  ggplot(aes(year, SWE)) + 
  geom_line() +
  # why is raw ccsm off?
  #geom_line(data = ccsm_dat %>%group_by(year) %>% summarise(SWE = sum(SWE)), color = 'blue') +
  #geom_line(data = recon_project_cesm %>%group_by(year) %>% summarise(SWE = sum(SWE)), color = 'blue') +
 # geom_line(data = recon_common_ccsm %>%group_by(year) %>% summarise(SWE = sum(SWE)), color = 'red') +
   # geom_line(data = recon_common_cesm %>%group_by(year) %>% summarise(SWE = sum(SWE)), color = 'green') +
  geom_line(data = recon_20c_time %>%group_by(year) %>% summarise(SWE = sum(SWE)), color = 'red') +
    geom_line(data = prism_dat %>%group_by(year) %>% summarise(SWE = sum(SWE)), color = 'orange') #+
 #  geom_line(data = cera_dat %>%group_by(year) %>% summarise(SWE = sum(SWE)), color = 'purple')


# so it looks like bias in the mean field strikes again -- another argument for maybe subtracting the cesm mean rather than the calibration mean, here doing that looks like it leads to biases. but wait, something looks wrong .... oh theres no area weighting
```
```{r}
recon_project_ccsm %>%
  filter(year < 1880 | year > 1975) %>%
  group_by(x, y, era = year > 1900) %>%
  summarise(SWE = mean(SWE)) %>%
  pivot_wider(names_from = era, values_from = SWE) %>%
  summarise(SWE = (`TRUE` - `FALSE`)) %>%
  #filter(SWE > -5) %>%
   ggplot() +
  geom_raster(aes(x, y, fill = SWE)) +
    geom_sf(data = states_wus, fill = NA, color = 'black') +
...
```{r}
plot_eofs(test, scaled = FALSE)
```


```{r}
cera_baseline <- cera_dat %>%
  left_join(areas_cera) %>%
  group_by(year) %>%
  summarise(SWE = sum(SWE * area))

ccsm_series <- recon_project_ccsm %>%
  left_join(areas_prism) %>%
  group_by(year) %>%
  summarise(SWE = sum(SWE * area))

cesm_series <- recon_project_cesm %>%
  left_join(areas_prism) %>%
  group_by(year) %>%
  summarise(SWE = sum(SWE * area))

ccsm_series_raw <- ccsm_dat %>%
  left_join(areas_cera) %>%
  group_by(year) %>%
  summarise(SWE = sum(SWE * area))

cesm_series_raw <- cesm_dat %>%
  left_join(areas_cera) %>%
  group_by(year) %>%
  summarise(SWE = sum(SWE * area))
```
...
```{r}
delta_preds <- delta(pred = filter(cera_dat, year >= 1982),
      obs = filter(prism_dat, year <= 2010),
      newdata = cera_dat)

delta_baseline_add <- left_join(mutate(delta_add_dat, x = round(x,5), y = round(y,5)), mutate(areas_prism, x = round(x,5), y = round(y,5)), by = c("x", "y")) %>%
  group_by(year) %>%
  summarise(SWE = sum(SWE * area, na.rm = TRUE))

delta_baseline_mult <- left_join(mutate(delta_mult_dat, x = round(x,5), y = round(y,5)), mutate(areas_prism, x = round(x,5), y = round(y,5)), by = c("x", "y")) %>%
  group_by(year) %>%
  summarise(SWE = sum(SWE * area, na.rm = TRUE))

delta_baseline_mult2 <- left_join(mutate(delta_mult_dat2, x = round(x,5), y = round(y,5)), mutate(areas_prism, x = round(x,5), y = round(y,5)), by = c("x", "y")) %>%
  group_by(year) %>%
  summarise(SWE = sum(SWE * area, na.rm = TRUE))
```
...
left_join(recon_20c_time, areas_prism, by = c("x", "y")) %>%
  ...
  geom_line(data = wus_stats, aes(color = 'red'), size = 1.1, alpha = .7) +
    ...
    geom_line(data = filter(cesm_series_raw, year > 1840), aes(color = 'green'), size = 1.1, alpha = .7) +
     ...
     # geom_line(data = cesm_series, aes(color = 'red'), size = 1.1, alpha = .7) +
 #geom_line(data = delta_baseline, color = 'green') +
  # geom_line(data = delta_baseline_mult, color = 'blue') +
 #  geom_line(data = delta_baseline_mult2, color = 'orange') +
...
sum(dof_l)^2 / sum(dof_l^2) # from bretheron et al 1999
```