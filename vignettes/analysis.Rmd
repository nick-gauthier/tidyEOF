...
```{r, fig.asp = .9}
recon <- left_join(prism_patterns$amplitudes, prism_patterns$eofs, by = c('PC' = 'EOF')) %>%
  mutate(PC = as.numeric(PC)) %>%
  #left_join(dplyr::select(prism_patterns$eigenvalues, PC, std.dev), by = 'PC') %>%
  mutate(anomaly = weight * amplitude) %>% # should be weight for reofs or?
