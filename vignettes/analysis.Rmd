...
predict_cca <- function(cca_mod, newdata, preds_patterns, obs_patterns, k_preds) {
  new_amps <- left_join(newdata, preds_patterns$climatology, by = c("x", "y")) %>%
    mutate(SWE = SWE - swe_mean) %>%
    dplyr::select(-swe_mean, -swe_sd) %>%
    dplyr::mutate(SWE = SWE * sqrt(cos(y * pi / 180))) %>%
  ...
  k <- length(cca_mod$cor)

  new_years <- unique(newdata$year)
  
  new_amps %*% 
      cca_mod$xcoef[,1:k, drop = FALSE] %*% # drop = FALSE prevents a vector returning when k = 1
    diag(cca_mod$cor, nrow = k) %*%  # nrow likewise rpevents weired behavior when k = 1
    solve(cca_mod$ycoef)[1:k,,drop = FALSE] %>%
    `rownames<-`(new_years) %>%
    as_tibble(rownames = 'year') %>% 
    gather(PC, amplitude, -year) %>% 
    mutate(year = as.numeric(year)) %>%
...
fit_kfold_cca <- function(k_preds, k_obs, preds, obs){
  ...
  train_obs_patterns <- map(1:5, ~ filter(folds, fold == .) %>%
                              anti_join(obs, ., by = 'year') %>% # training years
                              get_patterns(k = k_obs))
  
  train_preds_patterns <- map(1:5, ~ filter(folds, fold == .) %>%
                               anti_join(preds, ., by = 'year') %>% # training years
                               get_patterns(k = k_preds))
  
  train_obs_amps <- map(train_obs_patterns, ~ .$amplitudes %>% 
                          mutate(PC = as.numeric(PC)) %>% # so spread doesn't lose order of columns
                          spread(PC, amplitude) %>%
                          select(-year) %>%
                          as.matrix())
  
  train_preds_amps <- map(train_preds_patterns, ~ .$amplitudes %>%
                            mutate(PC = as.numeric(PC)) %>% 
                            spread(PC, amplitude) %>%
                            select(-year) %>%
                            as.matrix() )
  ...
  cca <- map2(train_preds_amps, train_obs_amps, cancor, xcenter = FALSE, ycenter = FALSE)
  
  # predict CCA on new data
  pmap_dfr(list(cca, test, train_preds_patterns, train_obs_patterns), predict_cca, k_preds = k_preds) %>% 
    arrange(x, y, year)
}
```
...
cv_gam <- expand_grid(k_predictor = 1:9, k_response = 1:9) %>%
  mutate(cv = map2(k_predictor, k_response, ~fit_kfold(.x, .y, preds = cera_dat, obs = prism_dat)))


cv_dat <- bind_rows(cv_gam, cv_cca, .id = 'type')
```
