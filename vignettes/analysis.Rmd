...
sst_sig <- brick(
  calc(sst_jfm, fun=function(x) cortest(x, k = 1)),
    calc(sst_jfm, fun=function(x) cortest(x, k = 2)),
    calc(sst_jfm, fun=function(x) cortest(x, k = 3)),
    calc(sst_jfm, fun=function(x) cortest(x, k = 4))) %>%
  `names<-`(paste0('PC', 1:4))

rasterToPoints(sst_sig, fun = function(x) x <= 0.05)
```
...
sst_cor %>% 
  ...
  scale_fill_distiller(name = 'Correlation', palette = 'RdBu', direction = 1, limits = c(-1,1)) +
...
geop_cor <- brick(
  ...
  calc(geop_jfm, fun=function(x) cor(c(x), prism_patterns[[5,2]]$amplitude)),
  calc(geop_jfm, fun=function(x) cor(c(x), prism_patterns[[6,2]]$amplitude))) %>%
  `names<-`(paste0('PC', 1:6))
```
...
geop_cor %>% 
  ...
  scale_fill_distiller(name = 'Correlation', palette = 'RdBu', direction = 1, limits = c(-1,1)) +
  coord_quickmap(ylim = c(-75,75), expand = FALSE) +
  geom_polygon(data = world, aes(x = long, y = lat, group = group), alpha = .2) +
  labs(title = 'Snow teleconnections', 
       subtitle = 'PC_cov correlation to JFM 500mb geopotential height',
       x = 'Longitude', y = 'Latitude') +
  theme_bw() +
  theme(legend.position = "bottom")
