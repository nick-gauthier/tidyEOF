...
```{r}
fit_kfold <- function(data, obs_val){
  ...
  cv <- map2_dfr(train, test, ~ fit_model(.x) %>% 
                            predict_patterns(.y) %>% 
...
```{r}
cv_dat <- expand_grid(k_cera = exp_test$k_cera, k_prism = exp_test$k_prism) %>%
  mutate(data = map2(k_cera, k_prism, ~prep_data(exp_test[[.x, "cera_patterns"]], exp_test[[.y, "prism_patterns"]])))  %>%
  mutate(cv = map2(data, k_prism, ~fit_kfold(.x, exp_test[[.y, "prism_patterns"]])))
```
...
rmse_dat <- cv_dat %>%
  ...
  unnest(cv) %>%
      ...
      summarise(mse =mean((total_obs - total_recon) ^ 2),
                rmse = sqrt(mse)) %>% 
  arrange(mse, rmse)
