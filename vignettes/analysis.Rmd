...
```{r, echo = FALSE, fig.asp = .8, fig.cap="A) Mean March SWE from PRISM/SNOTEL observations, 1982-2017. B) Mean March SWE from CERA-20C Reanalysis, 1901-2010. C) Standard deviation of March SWE from PRISM/SNOTEL observations, 1982-2017. B) Standard deviation of March SWE from CERA-20C Reanalysis, 1901-2010."}
...
c <- ggplot(get_climatology(prism_dat)) +
  geom_raster(aes(x, y, fill = swe_sd)) +
  geom_sf(data = states_wus, fill = NA, color = 'black') +
  theme_void() +
  scale_fill_viridis_c()

d <- ggplot(get_climatology(cera_dat)) +
  geom_raster(aes(x, y, fill = swe_sd)) +
  geom_sf(data = states_wus, fill = NA, color = 'black') +
  theme_void() +
  scale_fill_viridis_c()

(a + c) / (b + d) + plot_annotation(tag_levels = 'A')
```
...
setup a 5 fold cross validation experiment for selecting k.

```{r}
exp_test <- tibble(k_cera = 1:10, k_prism = 1:10) %>%
...
fit_kfold <- function(k_preds, k_obs){
  preds <- exp_test[[k_preds, "cera_patterns"]]
  obs <- exp_test[[k_obs, "prism_patterns"]]
