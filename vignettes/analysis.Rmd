...
# temporary fix until broom fixes tidy.prcomp
...
load('../data.Rdata')
cesm_dat <- read_csv('../cesm_dat.csv') %>%
  select(-area)
...
a <- prism_dat %>%
  ...
  geom_raster(aes(x, y, fill = swe_mean)) +
  geom_sf(data = states_wus, fill = NA, color = 'black') +
   scale_fill_scico(palette = 'davos', direction = -1, name = 'SWE\n(mm)') +
  theme_void()
...
(a + (b / c + plot_layout(guides = 'collect') )) + plot_layout( widths = c(1.5,1)) + plot_annotation(tag_levels = 'A') & theme(legend.position = 'bottom')

ggsave('swe_comparison.png', width = 5, height = 4)
```
...
cesm_patterns <- get_patterns(cesm_dat %>% filter(year > 1800), k = nmodes)
```
...
recon_20c_gam <- couple_patterns(get_patterns(cera_dat, 4), get_patterns(prism_dat, 6)) %>%
...
delta_baseline_add <- left_join(mutate(delta_add_dat, x = round(x,5), y = round(y,5)), mutate(areas, x = round(x,5), y = round(y,5)), by = c("x", "y")) %>%
  group_by(year) %>%
  summarise(SWE = sum(SWE * area, na.rm = TRUE))

delta_baseline_mult <- left_join(mutate(delta_mult_dat, x = round(x,5), y = round(y,5)), mutate(areas, x = round(x,5), y = round(y,5)), by = c("x", "y")) %>%
  group_by(year) %>%
  summarise(SWE = sum(SWE * area, na.rm = TRUE))

delta_baseline_mult2 <- left_join(mutate(delta_mult_dat2, x = round(x,5), y = round(y,5)), mutate(areas, x = round(x,5), y = round(y,5)), by = c("x", "y")) %>%
  group_by(year) %>%
  summarise(SWE = sum(SWE * area, na.rm = TRUE))
```
...
```{r}
left_join(recon_20c_time, areas, by = c("x", "y")) %>%
  group_by(year) %>%
  summarise(SWE = sum(SWE * area)) %>%
  ggplot(aes(year, SWE)) +
...
delta_add_dat <- delta_add %>% setNames(1901:2010) %>%
  as.data.frame(xy = TRUE, na.rm = TRUE, long = TRUE) %>%
  rename(year = layer, SWE = value) %>%
  mutate(year = parse_number(year))

delta_mult_dat <- delta_mult %>% setNames(1901:2010) %>%
  as.data.frame(xy = TRUE, na.rm = TRUE, long = TRUE) %>%
  rename(year = layer, SWE = value) %>%
  mutate(year = parse_number(year))

delta_mult_dat2 <- delta_mult2 %>% setNames(1901:2010) %>%
  as.data.frame(xy = TRUE, na.rm = TRUE, long = TRUE) %>%
  rename(year = layer, SWE = value) %>%
  mutate(year = parse_number(year))
```
