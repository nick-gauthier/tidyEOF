...
```{r}
prism_patterns <- get_patterns(prism_pcs, prism_dat, prism_eigs, k = 4)
```
...
swe_recon <- prism_patterns %>%
  ...
  mutate(SWE = anomaly + swe_mean)# * swe_sd + swe_mean) 
...
swe_recon %>%
  ...
  gather(type, SWE, SWE_recon:SWE_obs) %>%
...
ggplot() +
  ...
  ggtitle('Differences in observed and reconstructed SWE')

swe_recon %>%
  left_join(prism_dat, by = c('x', 'y', 'year'), suffix = c('_recon', '_obs')) %>%
  filter(year <= 1985) %>%
  mutate(error = (SWE_recon - SWE_obs) / swe_sd) %>%
filter(year <= 1985) %>% 
ggplot() +
  ...
  theme_void() +
  facet_wrap(~year) +
  scale_fill_scico(palette = 'vik', direction = -1, name = 'Standard deviations', limits = c(-10, 10)) +
...
```{r, warning = FALSE}
sst_cor <- brick(
  calc(sst_jfm, fun=function(x) cor(c(x), prism_patterns[[1,2]]$amplitude)),
  calc(sst_jfm, fun=function(x) cor(c(x), prism_patterns[[2,2]]$amplitude)),
  calc(sst_jfm, fun=function(x) cor(c(x), prism_patterns[[3,2]]$amplitude)),
  calc(sst_jfm, fun=function(x) cor(c(x), prism_patterns[[4,2]]$amplitude))) %>%
  `names<-`(paste0('PC', 1:4))
```
...
sst_sig <- brick(
  calc(sst_jfm, fun=function(x) fdr(x, k = 1)),
    calc(sst_jfm, fun=function(x) fdr(x, k = 2)),
    calc(sst_jfm, fun=function(x) fdr(x, k = 3)),
    calc(sst_jfm, fun=function(x) fdr(x, k = 4))) %>%
  `names<-`(paste0('PC', 1:4))
...
geop_cor <- brick(
  calc(geop_jfm, fun=function(x) cor(c(x), prism_patterns[[1,2]]$amplitude)),
  calc(geop_jfm, fun=function(x) cor(c(x), prism_patterns[[2,2]]$amplitude)),
  calc(geop_jfm, fun=function(x) cor(c(x), prism_patterns[[3,2]]$amplitude)),
  calc(geop_jfm, fun=function(x) cor(c(x), prism_patterns[[4,2]]$amplitude))) %>%
  `names<-`(paste0('PC', 1:4))


geop_sig <- brick(
  calc(geop_jfm, fun=function(x) fdr(x, k = 1)),
  calc(geop_jfm, fun=function(x) fdr(x, k = 2)),
  calc(geop_jfm, fun=function(x) fdr(x, k = 3)),
  calc(geop_jfm, fun=function(x) fdr(x, k = 4))) %>%
  `names<-`(paste0('PC', 1:4))
...
livneh_raw <- brick('~/Downloads/LIVNEH/swe.mon.mean.nc')
livneh_indices <- getZ(livneh_raw) %>%
    str_detect(., '-03-') %>%
    which()

livneh_dat <- subset(livneh_raw, livneh_indices) %>%
  rotate() %>%
  crop(bbox)%>%
...
Run the principal components analysis.
```{r}
cera_pcs <- calc_pcs(cera_dat, scale = FALSE)
```
...
```{r, echo = FALSE}
plot_scree(cera_eigs, 4)
```
```{r}
cera_patterns <- get_patterns(cera_pcs, cera_dat, cera_eigs, k = 6, mask = FALSE)
```
