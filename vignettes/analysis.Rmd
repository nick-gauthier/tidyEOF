...
```
```{r}
corrplot::corrplot(can_cor$xcoef)
corrplot::corrplot(can_cor$ycoef)
```
...
train_obs <- map(1:5, ~ filter(folds, fold == .) %>%
  ...
  pmap_dfr(list(train_pred, train_obs, test), cca.predict, k.x = k_preds, k.y = k_obs)
}

cv_cca <- expand_grid(k_predictor = 1:9, k_response = 1:9) %>%
