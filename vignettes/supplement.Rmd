...
```{r}
 ...
 test <- get_fdr(sst_jfm, prism_patterns)#get_fdr(prism_clim['tmean'], prism_patterns)
test %>% st_apply('PC', adjust) %>% plot
test %>% st_apply('PC', function(x) sum(!is.na(x)))
  plot((test %>% st_apply('PC', adjust)) < 0.1)
  
  test[[1]] %>% is.na %>% `!`() %>% sum(na.rm = TRUE)
  slice(test, 'PC', 1) %>%.[[1]] %>% is.na %>% `!`() %>% sum(na.rm = TRUE)
# test %>% st_cast('MULTIPOINT') %>% st_transform(prj) %>% st_cast('LINESTRING') %>% plot
# test %>%  st_transform(prj) %>% plot
#
# plot(test)
# st_is_valid(test)
# ggplot() +
#   geom_sf(data = test ) +
#   facet_wrap(~PC) +
#  coord_sf(crs = prj)#"+proj=moll +lon_0=180")
# # this works, but need a way to loop through everything.
# ggplot() +
#   geom_sf(data = remove_missing(st_as_sf(test, long = TRUE) )) +
#   facet_wrap(~PC) +
#   coord_sf(crs = prj)#"+proj=moll +lon_0=180")
# ggplot() +
#   geom_stars(data = st_xy2sfc(test,as_points = TRUE), downsample = c(5,5,1)) +
#   facet_wrap(~PC) +
#   coord_sf(crs = prj)
#
# st_apply(prism_clim['ppt'], 'time', function(x) sum(!is.na(x))) %>% .[[1]]
#
#
#
# test %>% plot
...
#
# ggplot() +
#   geom_stars(data = st_warp(geop[,,,1], dest = ref, use_gdal = TRUE, method = 'bilinear'))
# ggplot() +
#   geom_stars(data = geop[,,,1:6]- st_apply(geop, 1:2, mean)) +
...
#
# ggplot() +
#   geom_stars(data = st_warp(geop[,,,1:6]- st_apply(geop, 1:2, mean), dest = ref)) +
...
#
# ggplot() +
#   geom_stars(data = st_warp(geop[,,,1:6]- st_apply(geop, 1:2, mean), dest = ref, use_gdal = TRUE, method = 'bilinear')) +
#   scale_fill_distiller(palette = 'RdBu', na.value = NA) +
#   facet_wrap(~band) +
#   coord_sf(crs = prj)
# ggsave('test3.png')
#
# ggplot() +
#   geom_stars(data = test < 0.1) +
#   facet_wrap(~PC)
#
# ggplot() +
#   geom_stars(data = test2 < 0.1) +
#   facet_wrap(~PC)
...
```{r}
tele <- (e/f/i/j) + plot_layout(guides = 'collect', heights = c(1.5,1.5,1, 1)) + plot_annotation(tag_levels = 'a', tag_prefix = '(', tag_suffix = ')') & theme(legend.position = 'bottom')
```

```{r}
ggsave('snow_teleconnections3.png',tele,  width = 7, height = 7)
```
