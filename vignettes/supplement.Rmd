...
cesm_patterns <- get_patterns(cesm_dat %>% filter(year > 1900,x < max(x), y > min(y)), k = n_modes)
swe_corr_cesm <- cesm_dat %>%
  filter(year > 1900,x < max(x), y > min(y)) %>% # is min/max check still necessary
  rename(value = SWE) %>%
  get_corrs(cesm_patterns)
...
```{r}
ggsave('snow_teleconnections.png', width = 10.5, height = 6, dpi = 600)
```
...
cesm_series <- predict_cca(get_patterns(filter(cera_dat, year >=1982) %>% semi_join(get_climatology(cesm_dat)), 5), 
            get_patterns(filter(prism_dat, year <= 2010), 4), 
...
```{r}
bind_rows('obs' = wus_stats %>% filter(year < 2005), 
          #'cera' = cera_baseline,
          'recon' = recon_series,
          'ccsm' = ccsm_series, .id = 'type') %>%
  ggplot(aes(SWE, fill = type)) +
  geom_density(alpha = .5)
```


```{r}
  ggplot(recon_series, aes(year, SWE)) +
  ...
  geom_line(data = wus_stats, aes(color = 'red'), size = 1.1, alpha = .7) +
    ...
    #geom_line(data = filter(cesm_series, year > 1840), aes(color = 'purple'), size = 1.1, alpha = .7) +
...
ggsave('anomalies_1997_unstandardized.png', height = 3.25, width = 6.5, dpi = 600)
```
