...
a / b + plot_layout(guides = 'collect') + plot_annotation(tag_levels = 'A') & theme(legend.position = 'bottom')
ggsave('eofs.png', height = 5, width = 7)
```

```{r}
prism_patterns$amplitudes %>% filter(PC == '3') %>%
  left_join(cera_patterns$amplitudes %>% filter(PC == '2'), by = c('year')) %>% summarise(cor(amplitude.x, amplitude.y, use = 'complete.obs'))
```
